...
task api:lint       # cargo clippy
task ui:lint        # ng lint
task api:fmt        # cargo fmt
task api:fmt-check  # cargo fmt --check
task api:test       # cargo test
task ui:test        # ng test --no-watch --browsers=ChromeHeadless
task ui:test-watch  # ng test (watch mode)
task api:update     # cargo update
task ui:update      # npm update
```
...
- ESLint is configured (`eslint.config.js`); run via `task ui:lint` or `task lint`
